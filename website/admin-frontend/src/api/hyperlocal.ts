...
export interface OfferRecord { _id: string; title: string; description?: string; category?: string; discountPercentage?: number; startsAt?: string; status: string; isActive?: boolean; offerPrice: number; originalPrice: number; isFeatured: boolean; expiresAt: string; moderationReason?: string; imageUrls?: string[]; cardDesign?: { templateId?: string; previewUrl?: string; canvas?: TemplateCanvasRecord; dynamicFields?: Record<string, unknown>; avatarId?: string; customizations?: Record<string, string | number | boolean> }; business?: BusinessRecord; city?: CityRecord; }
export interface HomeShowcase { banner: { imageUrl: string; title: string; subtitle: string; buttonText: string }; trendingOfferIds: string[]; }
export const getHomeShowcase = () => client.get<{ showcase: HomeShowcase; selectedOffers: OfferRecord[] }>('/hyperlocal/home-showcase').then((r) => r.data);
export const listShowcasePosters = (params: { page: number; limit: number; search?: string }) => client.get<Paginated<OfferRecord>>('/hyperlocal/home-showcase/posters', { params }).then((r) => r.data);
export const updateHomeShowcase = (showcase: HomeShowcase) => client.put('/hyperlocal/home-showcase', showcase).then((r) => r.data.showcase as HomeShowcase);
...
export interface WorkerRecord { _id: string; name: string; phone: string; whatsapp?: string; categories: CategoryRecord[]; city: CityRecord; serviceAreas?: string[]; availability: string; isActive: boolean; verificationStatus: string; ratingAverage: number; completedBookings: number; user?: { _id: string; name?: string; phone: string }; }
...
export interface PaymentRecord { _id: string; orderId: string; type: string; amount: number; status: string; providerPaymentId?: string; failureReason?: string; user?: { name?: string; phone: string }; business?: BusinessRecord; booking?: BookingRecord; createdAt: string; }
...
export type TemplateElementType = 'text' | 'image' | 'shape' | 'rectangle' | 'circle' | 'line' | 'button' | 'badge' | 'icon' | 'divider' | 'group';
export type TemplateElementRecord = { id: string; type: TemplateElementType; key?: string; field?: string; text?: string; content?: string; imageUrl?: string; src?: string; avatarId?: string; position?: { x: number; y: number }; size?: { width: number; height: number }; x: number; y: number; width: number; height: number; zIndex?: number; visible?: boolean; locked?: boolean; color?: string; backgroundColor?: string; fontSize?: number; fontWeight?: string; fontFamily?: string; fontStyle?: 'normal' | 'italic'; letterSpacing?: number; lineHeight?: number; numberOfLines?: number; textAlign?: 'left' | 'center' | 'right'; textAlignVertical?: 'top' | 'center' | 'bottom'; textDecorationLine?: 'none' | 'underline' | 'line-through' | 'underline line-through'; textTransform?: 'none' | 'uppercase' | 'lowercase' | 'capitalize'; borderRadius?: number; borderWidth?: number; borderColor?: string; borderStyle?: 'solid' | 'dotted' | 'dashed'; rotation?: number; opacity?: number; resizeMode?: 'cover' | 'contain' | 'stretch'; editable?: boolean; style?: Record<string, unknown> };
export type TemplateCanvasBackground = { type?: 'solid' | 'gradient' | 'linear-gradient' | 'image' | 'texture'; color?: string; from?: string; to?: string; direction?: string; angle?: number; colors?: string[]; imageUrl?: string; opacity?: number; overlayColor?: string; overlayOpacity?: number };
...
export const deleteCategory = (id: string) => client.delete(`${base}/service-categories/${id}`).then((r) => r.data.category);
...
export const deletePlan = (id: string) => client.delete(`${base}/plans/${id}`).then((r) => r.data.plan);
...
export const createDemoWorkers = (cityId: string, categoryIds: string[]) => client.post(`${base}/workers/demo`, { cityId, categoryIds }).then((r) => r.data as { requested: number; created: number; skipped: number });
export const setDummyProviderNumbers = () => client.post(`${base}/workers/dummy-numbers`).then((r) => r.data as { updated: number });
...
export const declinePayment = (id: string, reason: string) => client.post(`${base}/commerce-payments/${id}/decline`, { reason }).then((r) => r.data.payment);
export const deletePayment = (id: string) => client.delete(`${base}/commerce-payments/${id}`).then((r) => r.data.payment);
