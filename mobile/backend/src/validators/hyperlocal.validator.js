...
const offerPayload = Joi.object({
  ...
  cardDesign: Joi.object({
    ...
    // An empty id means the design has no fixed avatar. Custom designs can
    // still include one or more avatars as independent canvas elements.
    avatarId: Joi.string().trim().allow('').max(80).required(),
    ...
    titleFontSize: Joi.number().integer().min(16).max(96),
    descriptionFontSize: Joi.number().integer().min(11).max(48),
