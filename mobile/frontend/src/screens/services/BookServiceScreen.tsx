...
export const BookServiceScreen: React.FC<Props> = ({ route, navigation }) => {
    ...
    try {
      ...
      if (place) {
        ...
        if (!locality.trim() && (place.district || place.subregion)) {
          const detectedArea = place.district || place.subregion || '';
          const availableArea = route.params.availableAreas?.find((area) => area.toLocaleLowerCase('en-IN') === detectedArea.toLocaleLowerCase('en-IN'));
          if (!route.params.availableAreas?.length || availableArea) setLocality(availableArea || detectedArea);
        }
      }
    } catch (error: any) {
  ...
  return <ScreenContainer><KeyboardAvoidingView style={styles.providerCopy} behavior={Platform.OS === 'ios' ? 'padding' : 'height'}><View style={styles.top}><Pressable onPress={navigation.goBack} style={styles.back}><MaterialCommunityIcons name="arrow-left" size={24} /></Pressable><Text style={styles.title}>Book {route.params.categoryName}</Text></View><ScrollView style={styles.providerCopy} contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled" keyboardDismissMode="on-drag">
    ...
    {route.params.availableAreas?.length ? <Pressable accessibilityRole="button" accessibilityLabel="Select service area" onPress={() => setAreaPickerOpen(true)} style={styles.areaSelect}><Text style={[styles.areaSelectText, !locality && styles.areaPlaceholder]} numberOfLines={1}>{locality || 'Choose an available area'}</Text><MaterialCommunityIcons name="chevron-down" size={22} color={theme.colors.textSecondary} /></Pressable> : <Input label="Area / locality" value={locality} onChangeText={setLocality} placeholder="Your area" />}
    ...
    <Modal transparent animationType="slide" visible={areaPickerOpen} onRequestClose={() => setAreaPickerOpen(false)}>
      <Pressable style={styles.modalBackdrop} onPress={() => setAreaPickerOpen(false)}>
        <Pressable style={styles.areaSheet} onPress={(event) => event.stopPropagation()}>
          <View style={styles.sheetHandle} />
          <Text style={styles.sheetTitle}>Select service area</Text>
          <Text style={styles.sheetHint}>Providers available in the selected area will be shown.</Text>
          <ScrollView style={styles.areaOptions} showsVerticalScrollIndicator={false}>
            {route.params.availableAreas?.map((area) => <Pressable key={area} onPress={() => { setLocality(area); setAreaPickerOpen(false); }} style={[styles.areaOption, locality === area && styles.areaOptionActive]}><Text style={[styles.areaOptionText, locality === area && styles.areaOptionTextActive]}>{area}</Text>{locality === area ? <MaterialCommunityIcons name="check-circle" size={21} color={theme.colors.primary} /> : null}</Pressable>)}
          </ScrollView>
        </Pressable>
      </Pressable>
    </Modal>
