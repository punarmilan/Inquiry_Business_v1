...
export const toOfferCardTemplate = (template: {
  ...
  id: typeof template?._id === 'string' && template._id ? template._id : `template-${Date.now()}`,
  name: typeof template?.name === 'string' && template.name.trim() ? template.name : 'Offer template',
  category: typeof template?.category === 'string' && template.category.trim() ? template.category : 'All',
  defaultAvatarId: typeof template?.avatarId === 'string' ? template.avatarId : '',
  primaryColor: /^#[0-9a-f]{6}$/i.test(template?.primaryColor || '') ? template.primaryColor : DEFAULT_OFFER_CARD_DESIGN.primaryColor,
  secondaryColor: /^#[0-9a-f]{6}$/i.test(template?.secondaryColor || '') ? template.secondaryColor : DEFAULT_OFFER_CARD_DESIGN.secondaryColor,
  layout: ['right', 'left', 'bottom', 'center'].includes(template?.layout) ? template.layout : DEFAULT_OFFER_CARD_DESIGN.layout,
  description: template.description,
  previewUrl: template.previewUrl,
  canvas: normalizeOfferTemplateCanvas(template?.canvas),
  dynamicFields: template?.dynamicFields && typeof template.dynamicFields === 'object' ? template.dynamicFields : {},
  editableFields: Array.isArray(template?.editableFields) ? template.editableFields : [],
